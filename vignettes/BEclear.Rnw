...
\author{Markus Merl, David Rasp*\\
*email: \texttt{Davod.j.Rasp@gmail.com}}
\date{Modified: September 28, 2018  Compiled: \today}
\maketitle
...
<<echo=TRUE>>=
pvals <- calcPvalues(ex.data, ex.samples, adjusted=TRUE, method="fdr")
@
...
<<echo=TRUE>>=
mdifs <- calcMedians(ex.data, ex.samples)
