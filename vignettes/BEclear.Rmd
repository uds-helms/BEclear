...
```{r}
knitr::kable(ex.data[1:10,1:5])
```

And one data.frame containing the assignment of samples to batches:

```{r}
knitr::kable(ex.samples[1:10,])
```

# Detecting of batch effects

For the detection of the batch effects we calculate the median difference from
a gene in a batch from the value of this gene in all other batches. Furthermore 
we use a non-parametric Kolmogorov-Smirnov test (`ks.test`) to  compare the
distribution of the gene in the batch and the other batches.
