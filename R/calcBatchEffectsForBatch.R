...
calcBatchEffectsForBatch <- function(batch, samples, data, 
                                     BPPARAM = SerialParam()) {
  log_debug(paste("Calculating the batch effect for batch", batch))

  log_debug("Subsetting data.table for batch")
  DT_batch <- samples[batch_id == batch][data
    , ,
    on = .(sample_id = sample),
    nomatch = 0
  ][, .(feature, beta.value)]
  log_debug("Subsetting data.table for others")
  ...
  log_debug(paste("Calculating the p-values for batch", batch))
