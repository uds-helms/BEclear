...
#' then the \code{\link{imputeMissingData}} function is used to predict the missing values
#' and at the end, possibly existing wrongly predicted entries (values lower 
#' than 0 or greater than 1) are corrected using the 
...
#' whole input matrix. See \code{\link{imputeMissingData}} and especially the details 
#' section of See \code{\link{imputeMissingData}} for more information about this feature.
...
#' to predict the missing entries in the data matrix. See \code{\link{imputeMissingData}}
...
correctBatchEffect <- function(data, samples, adjusted=TRUE, method="fdr", 
                               rowBlockSize=60, colBlockSize=60, epochs=50,
    ...
    predicted <- imputeMissingData (cleared, rowBlockSize, colBlockSize,
        epochs, outputFormat, dir, BPPARAM = BPPARAM)
